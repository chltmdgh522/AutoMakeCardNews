...
public interface CardNewsSpringDataRepository extends JpaRepository<CardNews, Long> {
    @Query("SELECT e FROM CardNews e WHERE e.trash = 'X' AND (:title IS NULL OR e.title LIKE %:title%) AND (:category IS NULL OR e.category = :category OR :category = '') ORDER BY CASE WHEN :selected = '최신' THEN e.cardNewsId END DESC, CASE WHEN :selected = '' THEN e.cardNewsId END DESC, CASE WHEN :selected = '과거' THEN e.cardNewsId END ASC")
    ...
    @Query("SELECT e FROM CardNews e WHERE e.trash = 'X' AND e.edit = 'X' AND (:category IS NULL OR e.category = :category OR :category = '') ORDER BY CASE WHEN :selected = '최신' THEN e.cardNewsId END DESC, CASE WHEN :selected = '' THEN e.cardNewsId END DESC, CASE WHEN :selected = '과거' THEN e.cardNewsId END ASC")

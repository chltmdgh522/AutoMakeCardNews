...
var swiperBottom = new Swiper('.swiper-bottom', {
    ...
    autoplay: {
        delay: 3000, // 자동 슬라이드
        reverseDirection: true, // 직접 제어하여 반대 방향으로
    },
});

// 자동으로 왼쪽으로 이동하게 만듦
setInterval(function () {
    swiperBottom.slidePrev(); // 왼쪽으로 이동
}, 3000); // 3초 간격으로 이동
...
document.querySelectorAll('.cloud2').forEach(cloud2 => {
    // 구름의 초기 랜덤 위치 설정
    const setRandomPosition = () => {
        const randomLeft = Math.random() * 100; // 0 ~ 100vw 범위에서 랜덤한 위치
        cloud2.style.left = `${randomLeft}vw`;
    };
    ...
    // 애니메이션이 끝날 때마다 랜덤 위치 재설정
    cloud2.addEventListener('animationiteration', setRandomPosition);
});
